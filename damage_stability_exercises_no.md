...
- Task:
  - Compare $GM_S$ reduction for:
    - Slack tank free-surface effect
    - Damaged flooded compartment
- Given:
  - Same barge and geometric location assumptions
- Deliverables:
  - Short derivation or argument
  - Quantitative comparison (if data provided)
...
| Case | $L$ (m) | $B$ (m) | $T$ (m) | $N$ | $l_d$ (m) | $T_S$ (m) | $GM_S$ (m) |
|---|---:|---:|---:|---:|---:|---:|---:|
| Base | 60 | 12 | 3.0 | 5 | 12.0 | 3.750 | 0.875 |
| Fewer compartments (larger damage length) | 60 | 12 | 3.0 | 3 | 20.0 | 4.500 | 0.717 |
| Wider barge | 60 | 14 | 3.0 | 5 | 12.0 | 3.750 | 2.031 |
| Higher intact draft | 60 | 12 | 3.5 | 5 | 12.0 | 4.375 | 0.731 |
...
With $LCF_S = 30.00\ \mathrm{m}$ from AP (midship), trim components $t_a$ and $t_f$ are each half the total:

$$
t_f = 0.00771\ \mathrm{m},\qquad t_a = 0.00771\ \mathrm{m}
$$
...
$$
T_{S,\max} \approx \max(T_A, T_F) = 3.1935\ \mathrm{m} < D = 6.0\ \mathrm{m}\ \Rightarrow\ \text{OK}
$$
...
Use the same barge and D1 values for a like-for-like comparison.

Reference damaged-flooding result from D1:

- $GM_S=0.939\ \mathrm{m}$

Estimate intact $GM$ for the same box barge before damage:

$$
KB\approx T/2=1.50\ \mathrm{m},\qquad I_{WL}=\frac{LB^3}{12}=8640\ \mathrm{m^4}
$$

$$
BM_T=\frac{I_{WL}}{\nabla}=\frac{8640}{2160}=4.000\ \mathrm{m}
$$

$$
GM_{intact}\approx KB+BM_T-KG=1.50+4.000-4.20=1.300\ \mathrm{m}
$$

So the damaged flooded compartment reduces metacentric height by approximately:

$$
\Delta GM_{damage}\approx 1.300-0.939=0.361\ \mathrm{m}
$$

For a slack tank, the usual idealized correction is a virtual rise of $G$:

$$
\Delta GM_{FSE}=\frac{i_{fs}}{\nabla}
$$

If an example slack tank has $i_{fs}=250\ \mathrm{m^4}$, then:

$$
\Delta GM_{FSE}=\frac{250}{2160}=0.116\ \mathrm{m}
$$

Comparison and interpretation:

- In this example, damaged flooding ($0.361\ \mathrm{m}$ GM loss) is about 3.1 times the slack-tank FSE loss ($0.116\ \mathrm{m}$).
- FSE mainly acts through a virtual rise in $G$, while damaged flooding changes buoyancy geometry ($KB$, $BM$, $LCB/LCF$) and can also induce trim.
- Therefore, a fully flooded damaged compartment is typically a broader and often more severe stability perturbation than a single slack tank.
...
5. Check acceptance criteria: $GM_S>0$ and $T_{S,max}<D$.
```
...
print(f"Check T_S,max < D: {max(TA, TF):.4f} < {D:.2f} -> {draft_ok}")
