...
| $\nabla$ | Intact displacement volume | mÂ³ |
| $\nabla_S$ | Damaged displacement volume | mÂ³ |
| $I_{WL}$ | Transverse waterplane second moment (intact) | mâ´ |
| $I_{WL_S}$ | Transverse waterplane second moment (damaged waterplane) | mâ´ |
| $I_F$ | Longitudinal flotation-plane second moment (intact) | mâ´ |
| $I_{F_S}$ | Longitudinal flotation-plane second moment (damaged waterplane) | mâ´ |
...
*Animation: Intact hydrostatics â€” displacement volume and metacentric radii.*
...
*Animation: Vertical shift of the centre of buoyancy â€” $KB_S$ after damage.*

Equations:

For a rectangular box barge:

$$
KB = \frac{T}{2} < KB_S = \frac{T_S}{2}
$$
...
*Animation: Longitudinal geometry after unsymmetric damage â€” $LCB_S$, $LCF_S$, and $BM_{L_S}$.*
...
## 5. Full Worked Example: 5 Equal Compartments, Compartment 4 Damaged
...
$$
a_{aft}=\frac{LCF_S}{L}=0.45,\qquad a_{forward}=\frac{L-LCF_S}{L}=0.55
$$
...
### 5.1 Final Results (Additional Example)
...
## 6. Common Mistakes and Sanity Checks

Common mistakes:

- Mixing units between $\mathrm{m}$, $\mathrm{m^2}$, $\mathrm{m^3}$, and tonnes.
- Letting displacement change after flooding in this model ($\nabla_S\neq\nabla$ or $\Delta_S\neq\Delta$).
- Confusing transverse and longitudinal waterplane second moments ($I_{WL_S}$ and $I_{F_S}$).
- Switching trim sign convention partway through the calculation.
- Checking only mean draft instead of the critical draft $\max(T_A,T_F)$.

Sanity checks before accepting the answer:

- $T_S$ should normally be greater than $T$ when buoyant volume is lost.
- Verify that displacement is actually conserved in the setup: $\nabla_S=\nabla$ and $\Delta_S=\Delta$.
- $GM_S$ will usually be lower than in intact condition for most damage cases.
- The trim sign should be consistent with the position of $LCG$ relative to $LCB_S$.

## 7. Quick Reference Algorithm

1. Gather inputs: $L$, $B$, $D$, $T$, $KG$, $LCG$, $l_d$, plus the damaged-condition hydrostatic quantities or geometry needed to determine $KB_S$, $LCB_S$, $LCF_S$, $I_{WL_S}$, and $I_{F_S}$.
2. Solve the symmetric damaged draft from the lost buoyant length:

$$
T_S = \frac{L}{L-l_d}T
$$

3. Set the damaged equilibrium state with conserved displacement:

$$
\nabla_S=\nabla,\quad \Delta_S=\Delta
$$

4. Compute the updated hydrostatic quantities:

$$
BM_{T_S}=\frac{I_{WL_S}}{\nabla},\quad BM_{L_S}=\frac{I_{F_S}}{\nabla},\quad GM_S=KB_S+BM_{T_S}-KG
$$

5. Compute the trimming moment and total trim:

$$
M_{trim}=\Delta \times (LCG-LCB_S),\quad MCT_{1cm_S}=\frac{\Delta \times BM_{L_S}}{100 \times L},\quad trim_{cm}=\frac{M_{trim}}{MCT_{1cm_S}}
$$

6. Distribute the trim about $LCF_S$ to obtain the trim components $t_a$ and $t_f$.
7. Compute the final drafts:

$$
T_A=T_S-t_a,\quad T_F=T_S+t_f
$$

8. Perform the acceptance checks: $GM_S>0$ and $\max(T_A,T_F)<D$.

## 8. References and Figure Placeholders

Possible supporting references:

- Class notes on hydrostatics, trim, and damage stability.
- Standard naval architecture texts covering the lost buoyancy method.
- Damage-stability rule references if the module is later extended from an idealized barge to more realistic ship cases.

Figure placeholders that can map to existing exports:

- Figure showing symmetric damaged equilibrium and the new draft $T_S$.
- Figure showing the reduction in transverse metacentric radius $BM_T$ after damage.
- Figure showing the change in $LCB_S$, $LCF_S$, and longitudinal $BM_L$ for unsymmetric damage.
- Figure showing trim moment, trim distribution, and the final drafts $T_A$ and $T_F$.
