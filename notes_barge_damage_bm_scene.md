# Notes — BargeDamageBMScene

## Scene metadata
- Scene: `BargeDamageBMScene`
- Source: `barge_damage_bm_scene.py`
...
- Displacement volume `∇` is unchanged (parallel sinkage is handled separately in `BargeDamageSubmergenceScene`).
- The intact waterplane area is `A_WL = L · B`; after damage only two-thirds remains.
- The moment of inertia of the waterplane area is taken **about the longitudinal (fore–aft) axis** through the centroid of the waterplane, which is the relevant axis for transverse (`BM_T`) calculations.

---

## Phase 1 — Intact plan view and waterplane area
...
- Visual cue:
  ...
  - Write label "Vannlinjeareal før skade" and equation `A_WL = L · B`.
- Narration (short):
  - The full waterplane spans the entire length `L` and beam `B`.
  - This gives the intact waterplane area as the product `L · B`.
- Key equations:
  - `$A_{WL} = L \cdot B$`
- Given values:
...
- Takeaway: The intact waterplane covers the full `L · B` area.

---

## Phase 2 — Damaged plan view and effective waterplane area
...
- Visual cue:
  ...
  - Write label "Effektivt vannlinjeareal etter skade" and equation `A_WL_S = (2/3) L · B`.
- Narration (short):
...
## Phase 3 — Second moment of area of the damaged waterplane
...
- Visual cue:
  - Write label "Vannlinjearealets treghetsmoment" and equation
    `I_WL_S = (1/12) · (2L/3) · B³`.
- Narration (short):
  - The transverse metacentric radius `BM_T` depends on the second moment of the waterplane area about the **longitudinal axis** (the fore–aft axis), not the transverse axis.
  - For each intact rectangular strip of length `L/3` and beam `B`, this moment of inertia is `(1/12) · (L/3) · B³`. With two such strips the result doubles.
  - The longitudinal axis convention means we integrate `y²` across the beam, giving the `B³` term.
- Key equations:
...
- Unit check:
  - `I_WL_S` in m⁴.
- Common pitfall:
  - Using `L³` instead of `B³` — the longitudinal axis runs fore–aft, so it is the **beam** dimension `B` that appears cubed.
...
- Takeaway: `I_WL_S` is always calculated about the fore–aft axis for transverse stability; this yields the `B³` dependence.

---

## Phase 4 — Transverse metacentric radius after damage
- Goal: Compute `BM_T_S` and compare with the intact value.
- Visual cue:
  - Write label "Tverrskips BM etter skade" and equation `BM_T_S = I_WL_S / ∇`.
...
- Narration (short):
  ...
  - Because `I_WL_S` is reduced while `∇` is unchanged, `BM_T_S` is smaller than in the intact condition.
...
- Unit check:
  - `BM_T_S` in meters.
- Common pitfall: Changing `∇` when computing `BM_T_S` — the displacement volume equals the intact draft times intact length times beam (parallel sinkage is separate).
- Takeaway: Flooding a compartment reduces `BM_T` because it cuts the waterplane second moment while leaving `∇` unchanged.
...
<ul>
  <li>Intact waterplane: A_WL = L · B &rarr; I_WL = (1/12) L B³ (about longitudinal axis).</li>
  <li>Damaged waterplane: middle compartment flooded, A_WL_S = (2/3) L · B.</li>
  <li>Damaged second moment (longitudinal axis): I_WL_S = (1/12)(2L/3)B³.</li>
  <li>Displacement volume ∇ is unchanged.</li>
  <li>Therefore BM_T_S = I_WL_S / ∇ &lt; BM_T.</li>
</ul>
```
